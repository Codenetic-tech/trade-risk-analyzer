...
const RealtimeFund: React.FC = () => {
  ...
  const SPREADSHEET_ID = '1znT2Zf3_GTBo5aBRaPuggYEGpppopJ2OTdkSDqPzx1Q';
  const API_KEY = 'AIzaSyARwRZdZvIwVGWsLK8A0OOzAxaLdcqNcRA';
  ...
  // Check if two rows are different (excluding internal flags)
  const areRowsDifferent = (row1: RealtimeData, row2: RealtimeData): boolean => {
    const keys = ['Time', 'UCC', 'Amount', 'Seg', 'Gateway', 'Order ID', 'BackOffice', 'Kambala', 'pre globe', 'allocation'];
    return keys.some(key => row1[key] !== row2[key]);
  };
  ...
  const fetchAllData = async (isAutoRefresh = false) => {
    try {
      ...
      const realtime = await fetchSheetData('REALTIME');
      ...
      if (isAutoRefresh && lastFetchedData.current.length > 0) {
        ...
        const newDataMap = new Map(realtime.map(row => [row._id!, getRowContentHash(row)]));
        ...
        const newDataIds = new Set(realtime.map(row => row._id));
        ...
        const sortedData = realtime.sort((a, b) => {
          const timeA = new Date(a.Time).getTime();
          const timeB = new Date(b.Time).getTime();
          return timeB - timeA;
        });
                ...
                {isInitialLoading ? (
                  <TableRow>
                    <TableCell colSpan={10} className="text-center py-8">
                      ...
                      <p className="mt-2 text-slate-600">Loading data from Google Sheets...</p>
                    </TableCell>
                  </TableRow>
                  ...
                  <TableRow>
                    <TableCell colSpan={10} className="text-center py-8">
                      ...
                      <p className="text-slate-600">
                        Make sure your Google Sheet is properly configured and shared publicly
                      </p>
                    </TableCell>
                  </TableRow>
