...
interface FileUploadModalProps {
  ...
  onFilesSelected: (kambalaFile: File | null, codeFile: File | null, nseGlobeFile: File | null) => void;
}
...
export const FileUploadModal: React.FC<FileUploadModalProps> = ({
  ...
  const [nseGlobeFile, setNseGlobeFile] = useState<File | null>(null);

  const handleFileSelect = (type: 'kambala' | 'code' | 'nseGlobe', file: File) => {
    if (type === 'kambala') {
      setKambalaFile(file);
    } else if (type === 'code') {
      setCodeFile(file);
    } else {
      setNseGlobeFile(file);
    }
  };

  const handleConfirm = () => {
    onFilesSelected(kambalaFile, codeFile, nseGlobeFile);
    onOpenChange(false);
    // Reset files
    setKambalaFile(null);
    setCodeFile(null);
    setNseGlobeFile(null);
  };
      ...
      <DialogContent className="max-w-6xl">
          ...
          <CardContent>
            {nseGlobeFile ? (
              <div className="text-center py-4">
                <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                  <div>
                    <p className="text-sm font-medium text-green-800">{nseGlobeFile.name}</p>
                    <p className="text-xs text-green-600">File uploaded successfully</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setNseGlobeFile(null)}
                    className="text-green-600 hover:text-green-700"
                  >
                    <X className="h-4 w-4" />
                  </Button>
              ...
              <div className="text-center py-8">
                <Upload className="mx-auto h-12 w-12 text-slate-400 mb-4" />
                <label htmlFor="nseglobe-upload-modal" className="cursor-pointer">
                  <span className="text-blue-600 hover:text-blue-700 font-medium">
                    Click to upload
                  </span>
                </label>
                <input
                  id="nseglobe-upload-modal"
                  type="file"
                  accept=".T0150,.csv,.txt"
                  className="hidden"
                  onChange={(e) => {
                    const file = e.target.files?.[0];
                    if (file) handleFileSelect('nseGlobe', file);
                  }}
                />
              </div>
            )}
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t">
          ...
          <Button 
            ...
            disabled={!kambalaFile || !codeFile || !nseGlobeFile}
