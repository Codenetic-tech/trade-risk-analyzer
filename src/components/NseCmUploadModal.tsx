...
interface NseCmUploadModalProps {
  ...
  onFilesSelected: (files: { risk: File | null; nse: File | null; nri: File | null; }) => void;
}

export const NseCmUploadModal: React.FC<NseCmUploadModalProps> = ({
  open,
  onOpenChange,
  onFilesSelected,
}) => {
  ...
  const handleConfirm = () => {
    onFilesSelected({ risk: riskFile, nse: nseFile, nri: nriFile });
    onOpenChange(false);
    // Reset files
    setRiskFile(null);
    setNseFile(null);
