...
import { TrendingUp, TrendingDown, DollarSign, Banknote, Calculator, Download, Settings, Edit, AlertTriangle } from 'lucide-react';
import { NseFoUploadModal } from './NseFoUploadModal';
import { NseFoTable } from './NseFoTable';
...
const NseFo: React.FC = () => {
  ...
  const handleFilesUploaded = async (files: { 
    ...
    try {
      const result = await processNseFoFiles(files, unallocatedFund);
  ...
  const exportOutputFile = () => {
    ...
    link.href = url;
    link.download = `90221_ALLOC_FO_${dateString}.T0001`;
    link.click();
  ...
  if (isProcessing) {
    return (
      <ModernLoading 
        ...
        subMessage="Analyzing futures and options data, risk allocations, and CC01 margins. This may take a few moments."
      />
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="border-b border-slate-200 pb-6">
        ...
        <p className="text-slate-600 mt-2">
          Upload Risk, NSE Globe, and CC01 files to analyze NSE F&O allocation differences
        </p>
      </div>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-red-600 flex items-center">
                ...
                Negative Short Total
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-red-700">
                ₹{(processedData.summary.negativeShortTotal / 100000).toFixed(2)} L
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      {processedData && (
        <div className="space-y-6">
          ...
          <NseFoTable data={processedData.data} />
        </div>
      )}
